...
export class DungeonRoom extends Room<FloorFieldState> {
    maxClients = 4;
    ...
    onCreate(options: any) {
        ...
        // ==========================================
        // ⚔️ CORE COMBAT & MOVEMENT HANDLERS
        // ==========================================
        ...
        this.onMessage("attack", (client, data) => {
            ...
            let killedAny = false;
            const attackRangeSq = 9.0; // 3.0^2
            const damage = 25; 

            // PERFORMANCE: Only check enemies in the immediate grid cell
            ...
            if (killedAny) this.checkWaveProgress();
        });

        // ==========================================
        // ✨ ABILITIES & STATUS HANDLERS
        // ==========================================

        this.onMessage("set_aura_style", (client, data) => {
            const player = this.state.players.get(client.sessionId);
            if (player) {
        ...
        this.onMessage("toggle_aura", (client) => {
            const player = this.state.players.get(client.sessionId);
            if (player) player.isAuraActive = !player.isAuraActive;
        });
        ...
        this.onMessage("useAbility", (client, data) => {
            ...
            if (data.abilityId === "meteor_strike" || data.abilityId === "void_eruption" || data.abilityId === "grand_cross") {
                radius = 6.5; baseDamage = 120; isTargetedAoE = true;
            } else if (data.abilityId === "seismic_slam" || data.abilityId === "holy_nova" || data.abilityId === "feast_of_absolution") {
                radius = 5.0; baseDamage = 60; isTargetedAoE = false;
            } else if (data.abilityId === "blood_harvest" || data.abilityId === "sunder" || data.abilityId === "divine_smite") {
                radius = 2.5; baseDamage = 85; isTargetedAoE = true;
            } else if (data.abilityId === "earth_spike") {
                radius = 2.0; baseDamage = 40; isTargetedAoE = true;
            } else if (data.abilityId === "healing_blossom" || data.abilityId === "aura_of_purity") {
            ...
            // PERFORMANCE: Spatial Grid search for AoE hit detection
            for (const enemy of this.enemyGrid.getNearby(originX, originZ, radius)) {
                if (distSq(enemy.x, enemy.y, originX, originZ) <= rSq) {
                    ...
                    if (data.abilityId === "blood_harvest" || data.abilityId === "sunder") {
                        if (enemy.afflictions) enemy.afflictions.set("Bleed", new AfflictionState());
                    }
                    if (data.abilityId === "void_eruption" || data.abilityId === "feast_of_absolution") {
                        if (enemy.afflictions) enemy.afflictions.set("Necrosis", new AfflictionState());
                    }
                    if (data.abilityId === "blinding_flare") enemy.stunnedTimer = 3.0;
                    if (data.abilityId === "earth_spike" || data.abilityId === "umbral_snare") enemy.rootedTimer = 3.0;
                    
                    this.broadcastNearby(enemy.x, enemy.y, 40, "playerAttacked", {
                        targetX: enemy.x, targetZ: enemy.y, damage: baseDamage, isCrit: Math.random() > 0.85
                    });
        ...
        this.onMessage("toggle_meditate", (client) => {
            const player = this.state.players.get(client.sessionId);
            if (player) player.isMeditating = !player.isMeditating;
        });
    ...
    onLeave(client: Client) {
        ...
        if (p) this.playerGrid.remove(p, p.x, p.y);
    ...
    private update(dt: number) {
        ...
        this.state.enemies.forEach((enemy: EnemyState, id: string) => {
            ...
            if (enemy.rootedTimer > 0) enemy.rootedTimer -= dtSec;

            let nearestPlayer: any = null;
            let minDistSq = Infinity;

            // PERFORMANCE: Throttle targeting. Enemies don't need to recalculate their target every 50ms.
            if (this.frameCount % 3 === 0) {
                this.state.players.forEach((p: any) => {
                    const dSq = distSq(p.x, p.y, enemy.x, enemy.y);
                ...
                // Use cached target between logic frames
            ...
            if (enemy.action === "telegraphing" || enemy.action === "recovering") return; 

            const attackRange = enemy.name === "Goblin King" ? 4.5 : (enemy.attackRadius || 2.5);
            ...
            if (minDistSq <= attackRange * attackRange) {
                ...
                const angleToPlayer = Math.atan2(enemy.targetY - enemy.y, enemy.targetX - enemy.x);
                let moveVecX = Math.cos(angleToPlayer) * speed;
                let moveVecY = Math.sin(angleToPlayer) * speed;

                const separationRadiusSq = 2.25; // 1.5^2
                const repulsionStrength = 8.0; 

                // PERFORMANCE: Throttled Spatial Grid check for Separation Flocking
                ...
                // Minor Vector Normalization Optimization
    ...
    private triggerEnemyAttack(enemy: EnemyState, enemyId: string) {
        ...
        this.broadcastNearby(enemy.x, enemy.y, 60, "enemyTelegraph", {
            id: enemyId, type: attackType, x: enemy.x, z: enemy.y, radius: radius, time: windUpTime
        });
        ...
        this.clock.setTimeout(() => {
            ...
            this.broadcastNearby(currentEnemy.x, currentEnemy.y, 60, "enemyAttackExecuted", {
                id: enemyId, type: attackType, x: currentEnemy.x, z: currentEnemy.y, radius: radius
            });

            const rSq = radius * radius;
            
            for (const p of this.playerGrid.getNearby(currentEnemy.x, currentEnemy.y, radius)) {
                if (distSq(p.x, p.y, currentEnemy.x, currentEnemy.y) <= rSq) {
                    p.hp -= damage; 
                    this.broadcastNearby(p.x, p.y, 40, "playerAttacked", { id: p.sessionId, targetX: p.x, targetZ: p.y, damage: damage });
                    if (p.hp <= 0) {
                        this.broadcast("underworld_death", { message: "You were slain by the Horde..." });
                    }
                }
            }

            this.clock.setTimeout(() => {
                if (this.state.enemies.has(enemyId)) this.state.enemies.get(enemyId)!.action = "chasing";
            }, isEnraged ? 400 : 800); 
    ...
    private checkWaveProgress() {
        ...
        if (this.state.enemies.size === 0 && !this.isTransitioning) {
            if (this.currentWave >= this.maxWaves) {
                ...
                this.broadcast("dungeon_cleared", { text: "DUNGEON CONQUERED! CLAIMING REWARDS..." });
                ...
                this.state.players.forEach((player: any, sessionId: string) => {
                    ...
                    if (client) {
                        if (player.inventory && !player.inventory.has("Legendary Void Blade")) {
                            ...
                            newItem.quantity = 1;
                            newItem.desc = itemDef ? itemDef.desc : "An apocalyptic weapon forged from concentrated shadow essence. +150 ATK.";
                            
                            player.inventory.set("Legendary Void Blade", newItem);
                            client.send("legendary_loot_acquired", { 
                                itemName: "Legendary Void Blade", icon: itemDef ? itemDef.icon : "🌌"
                            });
                        }
                    }
                });

                this.clock.setTimeout(() => { this.broadcast("teleport_to_town"); }, 5000);
            } else {
                this.startNextWave();
            }
        }
    ...
    private startNextWave() {
        ...
        this.broadcast("dungeon_announcement", { text: `WAVE ${this.currentWave} APPROACHES!` });
        this.syncDungeonUI();

        this.clock.setTimeout(() => {
            if (!this.state) return; 
            
            let playerX = 0; let playerZ = 0;
            if (this.state.players.size > 0) {
                const p = Array.from(this.state.players.values())[0] as PlayerState;
                playerX = p.x; playerZ = p.y;
            }
            ...
            for (let i = 0; i < enemyCount; i++) {
                ...
                if (this.currentWave === this.maxWaves && i === 0) {
                    ...
                    enemy.maxHp = 1500; enemy.hp = 1500;
                    enemy.damage = 45; enemy.speed = 6;
                } else {
                    enemy.name = "Cave Goblin";
                    enemy.maxHp = 100 + (this.currentWave * 20); enemy.hp = enemy.maxHp;
                    enemy.damage = 10 + (this.currentWave * 2); enemy.speed = 8 + Math.random() * 2;
                }
    ...
    private spawnBossMinions(bossX: number, bossZ: number, count: number) {
        for (let i = 0; i < count; i++) {
            ...
            minion.maxHp = 150; minion.hp = 150;
            minion.damage = 20; minion.speed = 9;
    ...
    private syncDungeonUI() {
        this.broadcast("dungeon_sync", {
            wave: this.currentWave, maxWaves: this.maxWaves, enemiesLeft: this.state.enemies.size, timeRemaining: 0
        });
    }

    onDispose() { console.log("DungeonRoom Disposed"); }
}